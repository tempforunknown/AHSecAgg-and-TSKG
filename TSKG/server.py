...
class server(object):
    def __init__(self, port: int,pairing):
        self.port = port
    ...
    def recoverMsg(self):
        ...
        for i in self.unmaskMsg:
            for j in self.u2:
                if(j == i):
                    continue
                if j not in sharesforFirst:
                    sharesforFirst[j] = []
                    sharesforSecond[j] = []
                    indexforFirst[j] = []
                    indexforSecond[j] = []
                if j not in self.u3:
                    sharesforFirst[j].append(Element(self.pairing,G1,value='0'+(hex(self.unmaskMsg[i][str(j)])[2:])))
                    indexforFirst[j].append(i)
                else:
                    sharesforSecond[j].append(Element(self.pairing,G1,value='0'+(hex(self.unmaskMsg[i][str(j)])[2:])))
                    indexforSecond[j].append(i)
        #print(indexforFirst)
        #print(sharesforFirst)
        #print(indexforSecond)
        #print(sharesforSecond)
        ...
        for i in sharesforSecond:
            if(len(sharesforSecond[i]) != 0):
                secret = TSrecoverSign(thres,indexforSecond[i],sharesforSecond[i],self.pairing) 
                prg = aesPrg(secret)
                #print(i,secret)
                for i in range(len(self.result)):
                    self.result[i] =(self.result[i]- prg.genRandint()) % DHp
        
        for i in sharesforFirst:
            if(len(sharesforFirst[i]) != 0):
                secret = TSrecoverSign(thres,indexforFirst[i],sharesforFirst[i], self.pairing) 
                #print(i,secret)
                for j in self.u3:
                    if(i == j):
                        continue
                    commonKey = binpow(self.pubkeys[j]['maskKey'], secret, DHp)
                    prg = aesPrg(commonKey)
                    #print(commonKey)
                    if(i > j):
                        for i in range(len(self.result)):
                            self.result[i] = (self.result[i] + prg.genRandint()) % DHp
