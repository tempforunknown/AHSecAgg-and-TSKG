...
class client(object):
    def __init__(self, idnum: int, port: int, nodelist,pairing):
        self.port = port
        self.idNum = idnum
        self.pairing = pairing
        self.secretInput = np.random.randint(1, upbound, size = dimension, dtype='int64')
        self.t = len(nodelist) // 3 + 1
        self.fir = Element.random(self.pairing, Zr)
        self.firstTS = ThresholdSignature(params, self.pairing,self.t,nodelist,self.fir)
        self.sec = Element.random(self.pairing, Zr)
        self.secTS = ThresholdSignature(params, self.pairing,self.t,nodelist,self.sec)

        self.secMaskKey = (self.secTS.signature(nonce,self.sec)) % DHp
        
        #print('sec',self.secMaskKey)
        
        self.privateKeyforMask = (self.firstTS.signature(nonce,self.fir)) % DHp
        #print(self.privateKeyforMask)
    ...
    def genSharesMsg(self, ids: list):
        ...
        nodeNum = len(ids)
        thres = nodeNum // 3 + 1
        self.secMaskKeyShares = self.secTS.genShares()
        self.maskKeyShares = self.firstTS.genShares()
        msg = dict()
        for i in range(len(ids)):
            if(int(ids[i]) == self.idNum):
                continue
            msg['mask'] = int(self.maskKeyShares[int(ids[i])])
            msg['sec'] = int(self.secMaskKeyShares[int(ids[i])])
    ...
    def genMaskMsg(self, ids: list) -> str:
        ...
        for i in ids:
            if(i == self.idNum):
                continue
            prg = aesPrg(self.MaskCommonKeys[i])
            if(self.idNum > i):
    ...
    def genRecoverMsg(self, ids: list) -> str:
        ...
        for i in self.u2:
            if(i == self.idNum):
                continue
            if i not in ids:
                whose[i] = TSsignature(self.pairing,nonce,self.sharesforMask[i])
            else:
                whose[i] = TSsignature(self.pairing,nonce,self.sharesforSec[i])
        send = dict()
...
'''fir = Element.random(pairing, Zr)
#secr = int(str(fir), 16) % DHp

firstTS = ThresholdSignature(params, pairing,2,[1,2,3,4],fir)
secr = (firstTS.signature(nonce,fir)) % DHp
secr = (firstTS.signature(nonce,int(str(fir), 16))) % DHp
shares = firstTS.genShares()
...
for i in shares:
    index.append(i)
    shares[i] = TSsignature(pairing,nonce,shares[i])
    shar.append(Element(pairing,G1,value='0'+hex(shares[i])[2:]))
secret = TSrecoverSign(2,index,shar,pairing) 
print(secr)
print(secret)
print(shar)'''
