...
#print(recon(2, [3,1], [3421704958917470909, 5445187844426793730], DHp))
class aesPrg():
...
class ThresholdSignature():
    def __init__(self,params,pairing,t,nodelist,secret) -> None:
        self.params = params
    ...
    def random_polynomial(self,intercept):                       #随即生成拉格朗日多项式
        """ Generates a random polynomial with positive coefficients.
        """
        
        coefficients = [intercept]
        for i in range(self.t - 1):
            random_coeff = Element.random(self.pairing,Zr)
            coefficients.append(random_coeff)
        return coefficients

    def get_polynomial_points(self,coefficients):            #生成子秘密
        """ Calculates the first n polynomial points.
            [  (1, f(1)), ... ,(n, f(n)) ]
        """
        points = {}
    ...
    def genShares(self) -> dict:
        coe = self.random_polynomial(self.secret)
        shares = self.get_polynomial_points(coe)
        return shares
    


    def signature(self,m,sk):                                   #子秘密S生成子签名
        
        return int(str(Element(self.pairing,G1, value=m**sk )), 16)

def TSmodular_lagrange_interpolation(t,index_get,pairing):               #生成拉格朗日系数
    lagrange_polynomial_l =[]
    if len(index_get)<t:
        return lagrange_polynomial_l
    else:
        index_get=index_get[0:t]
        #ze=Element.zero(pairing,Zr)
        for i in index_get:
            # evaluate the lagrange basis polynomial l_i(x)
            numerator, denominator = Element.one( pairing, Zr ), Element.one( pairing, Zr )
            for j in index_get:
                # don't compute a polynomial fraction if i equals j
                if i == j:
                    continue
                ...
                denominator = (denominator * (i - j))
            # get the polynomial from the numerator + denominator mod inverse
            lagrange_polynomial_l.append(numerator * Element.__invert__(denominator))
        return lagrange_polynomial_l

def TSrecoverSign(t,index_get,sig_get,pairing):                                  #合成总签名
    li=TSmodular_lagrange_interpolation(t,index_get,pairing)
    R2=Element.one(pairing,G1)
    min_r=min(len(sig_get),len(li))
    for i in range(min_r):
        R2=R2*(sig_get[i]**li[i])
    res = (int(str(R2), 16)) % DHp
    return res

def TSsignature(pairing,m,sk):                                   #子秘密S生成子签名
    res = (int(str(Element(pairing,G1, value=m**sk )), 16))
    return res
